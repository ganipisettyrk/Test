...
export class CommonService {
        ...
        res => {
          if (res && res.isLoggedIn) {
            if (res.isHeaderUser && this.deviceService.isMobile()) {
              if (!localStorage.getItem("isHeaderUser")) {
                localStorage.setItem("isHeaderUser", "true");
              }
              if (this.isTefSpainHEEnabled() && res.encryptedMsisdn && null != res.encryptedMsisdn) {
  ...
  private isMsisdnAvailableInLocal(): Observable<boolean> {
    ...
    return new Observable((observer) => {
      ...
      if (this.deviceService.isMobile()) {
        ...
        let isHeaderUser: boolean = localStorage.getItem("isHeaderUser") == 'true';
        if (null != encryptedMsisdn && encryptedMsisdn.length > 0) {
          ...
          if (null == updated || updated != "true") {
            this.updateMsisdnDetails(encryptedMsisdn, tokenKey, isHeaderUser, true, encryptedUUID)
              .subscribe(res => {
                if (res) {
  ...
  public updateMsisdnDetails(encryptedMsisdn: string, tokenKey: string, isHeaderUser: boolean,
    fromLocalStorage: boolean, encryptedUUID: string): Observable<any> {
    ...
    return new Observable((observer) => {
      ...
      params.push({ paramName: 'tokenHeader', paramValue: isHeaderUser });

      this.httpRequest.post("updatedetails", null, params).subscribe(
        response => {
          if (null != response) {
            if (response.result == "success") {
              let trimCountryCodeForHeaderUser: boolean = this.translate.instant("pwa.trim.country.code.for.header.user") == 'true';
              if (trimCountryCodeForHeaderUser && this.deviceService.isMobile) {
                if (response.encryptedMsisdn && response.encryptedMsisdn != encryptedMsisdn) {
                  localStorage.removeItem("token");
                  this.setEncryptedMsisdnInLocalStorage(response.encryptedMsisdn, response.isHeaderUser);
                }
              }
  ...
  skipLoginForSelectedPages(currentPage: string): boolean {
    if (!currentPage || currentPage == null) {
      currentPage = "home";
    } else if (currentPage.startsWith("/")) {
      currentPage = currentPage.replace("/", "");
    }
    let skipLoginPages: string[] = this.translate.instant("pwa.exclude.login.for.pages").split(",");
    for (let i = 0; i < skipLoginPages.length; i++) {
      if (currentPage == skipLoginPages[i]) {
  ...
  checkIfReloadRequired(): boolean {
    let isLoggedIn: boolean = sessionStorage.getItem("loggedIn") == "true";
    if (isLoggedIn) {
      return false;
    }
    let currentUrl = window.location.href;
    let isUrlToBeSkipped = this.skipLoginForSelectedPages(this.getUpdatedPathUrl(currentUrl));
    if (isUrlToBeSkipped) {
      return false;
    }
    else {
      let isReloadSet: boolean = sessionStorage.getItem("reloadRequired") == 'true';
      if (isReloadSet) {
  ...
  getSafeValue(value: string): string {
    if (value) {
      let safeval = this.sanitizer.sanitize(SecurityContext.HTML, value);
      return safeval;
    }
    return value;
  }
