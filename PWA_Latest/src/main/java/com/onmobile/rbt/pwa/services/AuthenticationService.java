...
public class AuthenticationService implements WebConstants {
	...
	public String generateOTP(String msisdn, String type, HttpServletRequest httpRequest,
		...
		String storeId = utilsService.getStoreIdWithStoreIdentification(type, httpRequest,
				session, device);
		if (storeId == null) {
		...
		CryptoPayloadDto cryptoPayload = utilsService.generateRequestBody(msisdn, null,
				session, "generateOTP", null);
		HttpEntity<CryptoPayloadDto> request = new HttpEntity<CryptoPayloadDto>(
				cryptoPayload);
	...
	public String validateOTP(String msisdn, String pin, HttpSession session,
			HttpServletRequest request, Device device) throws StoreServerException {
		...
		int maxWrongOTPAttempts = applicationResource.getIntParameter(session,
				"pwa.wrong.otp.attempts", 3);
		...
		CryptoPayloadDto cryptoPayload = utilsService.generateRequestBody(msisdn, pin,
				session, "validateOTP", null);

		try {
			authenticationToken = voltronInterface.hitVoltronAPI(session, url,
					REQUEST_POST, cryptoPayload, AuthenticationToken.class);
		} catch (StoreServerException storeException) {
		...
		if (null != authenticationToken) {
			if (authenticationToken.getToken() != null
					|| !authenticationToken.getToken().isEmpty()) {

				boolean validateHmac = utilsService.validateHmacResponse(
						authenticationToken.getToken(), authenticationToken.getHmac(),
						session);
				if (!validateHmac) {
				...
				logger.info(
						"User with MSISDN : " + msisdn + " authenticated successfully!!");
				...
				responseWrapper = utilsService.updateUUIDInResponse(session,
						responseWrapper);
				return webUtils.obtainJSONFromObject(responseWrapper);
			}
			responseWrapper.setResult(INVALID_USER);
	...
	public String isUserLoggedIn(HttpServletRequest request, Device device) {
		...
		if (msisdn != null && !msisdn.isEmpty()) {
			...
			String encryptedMsisdn = utilsService
					.getTefSpainSpecificEncryyptedMsisdn(request, device);
			if (null != encryptedMsisdn && !encryptedMsisdn.isEmpty()) {
				isUserLoggedInStatus = true;
				responseWrapper.setEncryptedMsisdn(encryptedMsisdn);
				responseWrapper = utilsService.updateUUIDInResponse(request.getSession(),
						responseWrapper);
			}
		}
	...
	public ResponseWrapper updateMsisdn(HttpServletRequest request, HttpSession session,
			Device device, String encryptedMsisdn, String tokenKey,
			boolean isHeaderUserFromLocalStorage, boolean fromLocalStorage,
		...
		if (utilsService.isAcessedFromMobileDevice(device)) {
			...
			if (null != encryptedMsisdn && !encryptedMsisdn.isEmpty()) {
				...
				if (null != decryptedMsisdn && !decryptedMsisdn.isEmpty()) {
					...
					String storeId = null;
					try {
						storeId = utilsService.getStoreIdWithStoreIdentification(null,
					...
					try {
						boolean isHeaderUser = isHeaderUser(request, session, device,
								decryptedMsisdn, tokenKey, storeId, encryptedUUID,
								responseWrapper);
						if (isHeaderUserFromLocalStorage && isHeaderUser) {
							// Not setting if OTP user from local storage
							logger.info("setting isHeaderUser: " + isHeaderUser);
							responseWrapper.setHeaderUser(isHeaderUser);
							session.setAttribute(HEADERMSISDN, isHeaderUser);
						}
					} catch (StoreServerException storeException) {
						logger.error("Something went wrong while checking header user: "
					...
					if (!error) {
						...
						boolean isMsisdnTrimmed = false;
						Object isTrimmedValue = session.getAttribute(IS_MSISDN_TRIMMED);

						if (null != isTrimmedValue) {
							isMsisdnTrimmed = (Boolean) isTrimmedValue;
							if(isMsisdnTrimmed) {
								session.removeAttribute(IS_MSISDN_TRIMMED);
							}
						}
						if(!isMsisdnTrimmed) {
	...
	private boolean isHeaderUser(HttpServletRequest request, HttpSession session,
			Device device, String msisdn, String tokenKey, String storeId,
			String encryptedUUID, ResponseWrapper responseWrapper)
		...
		try {
			...
			token = utilsService.getAuthenticationToken(session, msisdn, storeId,
					encryptedUUID, null, null, null, null);

			if (null != token) {

				String trimCountryCodeForHeaderUser = applicationResource.getParameter(
						session, "pwa.trim.country.code.for.header.user");
			
				if (Boolean.parseBoolean(trimCountryCodeForHeaderUser)) {
					logger.info("Trim Country code enabled:");

					String userInfo = utilsService.getUserInfo(request, session, device);

					if (null != userInfo) {
						String userMsisdn = utilsService.getMsisdnFromUserInfo(userInfo);
						if (null != userMsisdn) {
							logger.info("Msisdn from User Info:" + userMsisdn);
							session.setAttribute(MSISDN, userMsisdn);
							session.setAttribute(IS_MSISDN_TRIMMED, true);

							isHeaderUser = true;
							String encryptedMsisdn = utilsService
									.getEncryptedMsisdn(userMsisdn, device);
		...
		logger.info("isHeaderUser response: " + isHeaderUser);
	...
	private String getDecryptedMsisdn(HttpSession session, String encryptedMsisdn) {
		...
		if (null != encryptedMsisdn && !encryptedMsisdn.isEmpty()) {
			try {
				...
				logger.error("decryption failed while updating msisdn, msisdn is:"
						+ encryptedMsisdn + e.getMessage());
			}
		}
		return decryptedMsisdn;
	}

	public String getDecryptedMsisdnForOoredo(HttpServletRequest request, Device device,
			String encryptedMdn, String correlatorId) {
		logger.info("Encrypted msisdn recieved: " + encryptedMdn);
		try {
			if (utilsService.isAcessedFromMobileDevice(device)) {

				encryptedMdn = URLDecoder.decode(encryptedMdn, "UTF-8");
				logger.info("Encrypted msisdn recieved After decode: " + encryptedMdn
						+ ", correlatorId: " + correlatorId);
				ResponseWrapper responseWrapper = new ResponseWrapper();

				HttpSession session = request.getSession();
				String decryptionKey = applicationResource.getParameter(session,
						"pwa.ooredo.decryption.key");
				String iv = applicationResource.getParameter(session,
						"pwa.ooredo.initialization.vector");

				String decryptedMsisdn = OoredoAESUtils.decrypt(encryptedMdn,
						hexStringToByteArray(decryptionKey), hexStringToByteArray(iv));
				...
				if (null != decryptedMsisdn) {
					...
					String dateFormat = applicationResource.getParameter(session,
							"pwa.ooredo.report.log.dateformat");
					OoredoOmanLogger.writeCDRLog(request, decryptedMsisdn, encryptedMdn,
							correlatorId, dateFormat);
					String encryptedMsisdn = utilsService
							.getEncryptedMsisdn(decryptedMsisdn, device);
			...
			logger.error(
					"decryption failed, msisdn is :" + encryptedMdn + e.getMessage());
		}
		return null;
	}
	...
	public String getDecryptedDataForContest(String encryptedData,
			String localStorageData, Integer storeIdIntVal, String encryptedUUID,
			HttpServletRequest request, HttpSession session) throws StoreServerException {
			...
			if (encryptedData != null) {
				...
				if (null != msisdn) {
					...
					if (sendResult) {
						...
						String storeId = Integer.toString(storeIdIntVal);
						if (null != storeId) {
	...
	public String postPwaLaunch(String language, String user_id,
			String subscription_status, String subscription_type, String operator,
			String circle, HttpServletRequest request, HttpSession session, Device device)
			throws StoreServerException {
		...
		try {
			...
			String url = applicationResource.getParameter(session,
					"pwa.app.launch.event.notifications.url");
			url = webUtils.getUpdatedVoltronUrl(session, url);
			logger.info("Going to hit PWA Launch url: " + url);
			String postResponse = voltronInterface.hitVoltronAPI(session, url,
					REQUEST_POST, pwaLaunchBeanStr, String.class);
	...
	public String getOperatorUserDetailsResponse(String token, String ctoken, String mode,
			String uuid, HttpServletRequest request, HttpSession session, Device device)
			throws StoreServerException {
		String res = getOperatorUserDetails(token, ctoken, mode, request, session,
				device);
		if (null != res) {
			ResponseWrapper responseWrapper = new ResponseWrapper();
			responseWrapper.setResult("success");
			responseWrapper.setEncryptedMsisdn(res);
			if (null != uuid && utilsService.isUUIDFeatureEnabled(session)) {
				session.setAttribute(WebConstants.CTOKEN, uuid);
				responseWrapper = utilsService.updateUUIDInResponse(session, responseWrapper);
			}
			return webUtils.obtainJSONFromObject(responseWrapper);
		}
	...
	public String getOperatorUserDetails(String token, String ctoken, String mode,
		...
		try {
			...
			String userInfo = voltronInterface.hitVoltronAPI(session, url, REQUEST_GET,
					null, String.class);
			if (null != userInfo) {
