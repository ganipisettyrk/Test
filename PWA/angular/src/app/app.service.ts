...
export class AppService {
  ...
  isBannerEnabled(url: string): Observable<boolean> {
    return new Observable((observer) => {
      let showBanner = false;
      if (url.startsWith('/home') || url == '/') {
        let loginDestinationUrl = sessionStorage.getItem("loginDestinationUrl");
        if (null != loginDestinationUrl && loginDestinationUrl.indexOf("home") == -1) {
          showBanner = false;
          observer.next(showBanner);
          observer.complete();
        } else {
          this.translate.get("pwa.home.enable.banner.section").subscribe(res => {
            showBanner = (res.trim() == 'true');
        ...
        this.translate.get("pwa.store.enable.banner.section").subscribe(res => {
          showBanner = (res.trim() == 'true');
          if (!showBanner) {
        ...
        this.translate.get("pwa.rtstore.enable.banner.section").subscribe(res => {
          showBanner = (res.trim() == 'true');
          if (!showBanner) {
            this.commonService.setIsInitialLoadingCompleted(true);
          }
        ...
        this.translate.get("pwa.more.enable.banner.section").subscribe(res => {
          showBanner = (res.trim() == 'true');
          observer.next(showBanner);
          observer.complete();
        });
