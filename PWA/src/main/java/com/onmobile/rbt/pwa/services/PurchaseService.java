...
public class PurchaseService implements WebConstants {
	...
	public String getPurchaseData(String contentId, String contentType,
			String contentSubtype, String extMode, String browsingLanguage,
		...
		if (url.contains("status")) {
			try {
				String status = utilService.getVoltronUserStatus(request, session, device,
						isRTContent);
		...
		try {
			purchaseData = voltronInterface.hitVoltronAPI(session, url, REQUEST_GET, null,
					String.class);
	...
	public String purchase(String contentId, String contentType, String caller,
			String retailPriceId, String subtype, String subscriptionId,
			String editSelectionType, String playruleId, String profilePlayRange,
			String browsingLanguage, String utm_params, String madeRefId, String madeContext, String extMode,
			HttpServletRequest request, HttpSession session, Device device)
			throws StoreServerException {
		logger.info("In PurchaseService: purchase()...");

		String userType = getUserNetworkType(session, device);

		String msisdn = utilService.getMsisdn(request, device);
		logger.info("PurchaseService, purchase(): Msisdn= " + msisdn);

		if (!"DEFAULT".equalsIgnoreCase(caller) && msisdn.equals(caller)) {
			logger.info("PurchaseService:: User etnered his own number, not continuing further...");
		...
		if ("allCaller".equals(editSelectionType)) {
			...
			try {
				mytunesService.deleteSpecificCallerSettings(playruleId, request, session,
						device);
		...
		String offlineCGFlow = applicationResource.getParameter(session,
				"pwa.enable.offline.cgflow", "false");
		logger.info("offlineCGFlow: " + offlineCGFlow);

		PurchaseComboWrapper comboObj = prepareComboObj(null, contentId, contentType,
				caller, retailPriceId, subtype, subscriptionId, playruleId,
				profilePlayRange, browsingLanguage, offlineCGFlow, userType, utm_params, madeRefId,
				madeContext, extMode, request, session);
		...
		PurchaseCombo purchaseCombo = (PurchaseCombo) webUtils.fromStringToObject(
				comboApiResp, PurchaseCombo.class);
		...
		if (purchaseCombo.getThirdpartyconsent() != null) {
			...
			if ("true".equalsIgnoreCase(offlineCGFlow) && NON_OPT_NETWORK.equals(userType)
				...
				rUrl += "&status_code=1";
				logger.info("Offline CG Flow is " + offlineCGFlow + " , userType: "
						+ userType + "and third party URL is null, so directly hitting R-URL "
						+ "by appending status_code=1 and URL: " + rUrl);

				String rUrlResp = voltronInterface.hitVoltronAPI(session, rUrl,
	...
	public PurchaseComboWrapper prepareComboObj(String uniqueId, String contentId,
			String contentType, String caller, String retailPriceId, String subtype,
			String subscriptionId, String playruleId, String profilePlayRange, 
			String browsingLanguage, String offlineCGFlow, String userType, 
			String utm_params, String madeRefId, String madeContext, String extMode,
	...
	public String getPaymentMethods(String contentId, String contentType, String caller,
			String retailPriceId, String subtype, String subscriptionId,
			String editSelectionType, String playruleId, String profilePlayRange,
			HttpServletRequest request, HttpSession session, Device device)
			throws StoreServerException {
		logger.info("Inside PurchaseService: getPaymentMethods()...");
		...
		PurchaseComboWrapper comboObj = prepareComboObj(uninqueKey, contentId,
				contentType, caller, retailPriceId, subtype, subscriptionId, playruleId,
				profilePlayRange, null, offlineCGFlow, userType, null, null, null, null, request,
				session);
	...
	private String getPaymentMethods(HttpSession session, String comboJsonString,
			String storeId, String token) throws StoreServerException {
		...
		String getPaymentMethodsApiResp = voltronInterface.hitVoltronAPI(session, url,
				REQUEST_POST, comboJsonString, String.class);

		logger.info("getPaymentMethods API response is: " + getPaymentMethodsApiResp);
		return getPaymentMethodsApiResp;
	}

	public String getComboRespAfterPayment(String uniqueId, String contentId,
			String extraInfo, HttpServletRequest request, HttpSession session,
